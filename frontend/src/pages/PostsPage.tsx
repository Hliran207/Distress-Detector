...
export function PostsPage() {
  ...
  const subreddit = subredditParam === '' ? undefined : subredditParam
  ...
  useEffect(() => {
    ...
    setError(null)
    api
      .listPosts({ limit, offset, label, subreddit })
      .then((d) => {
  ...
  }, [limit, offset, labelParam, subredditParam])
  ...
  function updateFilters(e: FormEvent<HTMLFormElement>) {
    ...
    const nextSubreddit = String(form.get('subreddit') ?? '').trim()

    const next = new URLSearchParams(params)
    next.set('limit', String(limit))
    next.set('offset', '0')
    nextLabel ? next.set('label', nextLabel) : next.delete('label')
    nextSubreddit ? next.set('subreddit', nextSubreddit) : next.delete('subreddit')
    setParams(next)
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <form onSubmit={updateFilters} className="rounded-lg border border-slate-200 p-4 grid gap-4 md_grid-cols-3">
        ...
        <div>
          <label className="block text-sm text-slate-600">Subreddit</label>
          <input
            name="subreddit"
            defaultValue={subredditParam}
            placeholder="e.g. LifeProTips"
            className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2 text-sm"
          />
          <div className="mt-1 text-xs text-slate-500">Case-sensitive (matches stored value).</div>
        </div>
